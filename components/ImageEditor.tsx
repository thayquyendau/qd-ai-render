...
export const ImageEditor: React.FC<ImageEditorProps> = ({
  ...
  const generateMaskImage = (): SourceImage | null => {
    ...
    // 1. Draw the strokes (which are on a transparent background)
    maskCtx.drawImage(drawingCanvas, 0, 0);

    // 2. Change the strokes to white (source-in fills only where pixels exist)
    maskCtx.globalCompositeOperation = 'source-in';
    maskCtx.fillStyle = 'white';
    maskCtx.fillRect(0, 0, maskCanvas.width, maskCanvas.height);

    // 3. Fill the background with black (destination-over fills only where pixels do NOT exist)
  ...
  const handleGenerate = async () => {
    ...
    setResultImage(null);
    try {
      // Request only 1 image (count = 1)
      const results = await editImage(image, maskImage, prompt, null, 1);

      if (results && results.length > 0) {
        const firstResult = results[0];
        setResultImage(firstResult);
        onEditComplete({ sourceImage: image, maskImage, prompt, resultImage: firstResult });
      } else {
