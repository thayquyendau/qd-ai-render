    ...
    return (
        <div>
            <div
                ...
                className={`relative group border-2 border-dashed rounded-lg p-4 flex items-center justify-center ${heightClass} mb-4 hover:border-blue-500 transition-colors cursor-pointer border-slate-600 bg-black/20`}
            ...
            >
                {sourceImage ? (
                    ...
                    <div className="text-center text-slate-400 pointer-events-none">
                        ...
                        <p className="text-xs mt-1">Hỗ trợ PNG, JPG, WEBP</p>
                    </div>
                )}
            </div>
...
const ImageViewerModal: React.FC<{ imageUrl: string | null; onClose: () => void; }> = ({ imageUrl, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-[500] p-4" onClick={onClose}>
            <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col relative animate-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
                <button onClick={onClose} className="absolute -top-4 -right-4 bg-blue-600 text-white rounded-full p-2 hover:bg-blue-700 transition-transform duration-200 hover:scale-110 z-[510]">
...
const UtilityRunner: React.FC<{ task: UtilityTaskDef; onBack: () => void; onEditRequest: (imageUrl: string) => void; isStandalone?: boolean; }> = ({ task, onBack, onEditRequest, isStandalone }) => {
    ...
    return (
        <div className="animate-in fade-in duration-500 relative z-10">
            {!isStandalone && (
                <button onClick={onBack} className="text-blue-400 hover:text-blue-300 mb-6 flex items-center gap-2 font-medium transition-transform hover:-translate-x-1">
            ...
            {isStandalone && (
                <div className="mb-8 text-center">
                    <h1 className="text-2xl font-bold font-orbitron text-blue-500 uppercase tracking-widest">{task.name} - AI POWERED</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start relative z-10">
                        ...
                        <div className="space-y-5">
                            {task.requiresMask && sourceImage1 ? (
                                <div className="space-y-3">
                                    <label className="text-xs text-blue-300 font-bold uppercase block">Vùng cần thay thế:</label>
                                    <div className="relative aspect-video bg-black/40 rounded-lg overflow-hidden border border-slate-700/50">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <input type="range" min="10" max="150" value={brushSize} onChange={(e) => setBrushSize(parseInt(e.target.value))} className="flex-grow h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                                    </div>
                                </div>
                            ) : (
                            ...
                            {task.id === 'virtual_staging' && (
                                <div className="space-y-4 bg-blue-900/10 p-4 rounded-lg border border-blue-900/20">
                                    <div>
                                        <label className="text-xs text-blue-300 font-bold uppercase mb-2 block">Loại Phòng:</label>
                                        <div className="flex flex-wrap gap-2">
                                            {STAGING_CONFIG.map(c => (
                                                <button key={c.type} onClick={() => {setStagingRoom(c.type); setSelectedItems([]);}} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${stagingRoom === c.type ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>{c.type}</button>
                                            ))}
                                        </div>
                                    </div>
                                    <div>
                                        <label className="text-xs text-blue-300 font-bold uppercase mb-2 block">Nội Thất Cần Thêm:</label>
                                        <div className="flex flex-wrap gap-2">
                                            {STAGING_CONFIG.find(c => c.type === stagingRoom)?.items.map(i => (
                                                <button key={i} onClick={() => setSelectedItems(p => p.includes(i) ? p.filter(x=>x!==i) : [...p,i])} className={`px-3 py-1 rounded border text-[11px] font-medium transition-all ${selectedItems.includes(i) ? 'border-blue-500 bg-blue-500/20 text-blue-100' : 'border-slate-700 text-slate-500 hover:text-slate-300'}`}>{i}</button>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                <label className="block text-xs font-bold text-blue-300 uppercase mb-2">Tỷ lệ khung hình</label>
                                <div className="grid grid-cols-3 gap-2">
                                    {["original", "1:1", "3:4", "4:3", "9:16", "16:9"].map((ratio) => (
                                        <button
                                            key={ratio}
                                            onClick={() => setAspectRatio(ratio)}
                                            className={`py-1.5 px-1 rounded text-[11px] font-bold transition-all border ${aspectRatio === ratio ? 'bg-blue-600 border-blue-400 text-white shadow-lg' : 'bg-slate-700 border-slate-600 text-slate-400 hover:bg-slate-600'}`}
                                        >
                            ...
                            <div className="relative z-0">
                                <label className="text-xs text-blue-300 font-bold uppercase mb-2 block">Mô tả thiết kế:</label>
                                <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full bg-slate-700 p-3 rounded-lg h-32 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="AI sẽ tự động mô tả..." />
                                {isAnalyzing && (
                                    <div className="absolute inset-0 flex items-center justify-center bg-slate-900/80 rounded-lg z-20 backdrop-blur-[1px]">
                                        <div className="animate-spin h-5 w-5 border-2 border-blue-500 border-t-transparent rounded-full mr-2"></div>
                                        <span className="text-xs font-bold text-blue-400">Đang phân tích...</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    </Section>
                    <button onClick={handleGenerate} disabled={isLoading || isAnalyzing || !sourceImage1} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-2xl disabled:bg-slate-800 flex items-center justify-center gap-2">
                ...
                <div className="lg:col-span-2 space-y-8 relative z-10">
                    <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 shadow-2xl p-6 rounded-xl flex flex-col min-h-[550px] relative overflow-hidden">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-lg font-semibold text-slate-300">Kết Quả Render</h2>
                            {resultImages.length > 0 && <span className="text-sm text-slate-400 font-orbitron">{resultImages.length} ảnh</span>}
                        </div>

                        <div className="flex-grow flex items-center justify-center bg-black/30 rounded-lg relative group overflow-hidden border border-slate-700/30 shadow-inner mb-4 min-h-[400px]">
                            {isLoading ? (
                                <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/60 backdrop-blur-md z-30">
                                    <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-400 mb-4"></div>
                                    <p className="text-blue-300 font-bold text-xs tracking-widest animate-pulse uppercase">Đang kiến tạo không gian...</p>
                                </div>
                                ...
                                <div className="relative w-full h-full flex items-center justify-center p-4">
                                    ...
                                    {upscalingIndex === selectedImageIndex ? (
                                        ...
                                        <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20 pointer-events-none group-hover:pointer-events-auto">
                                            <button onClick={() => setFullscreenImg(resultImages[selectedImageIndex])} className="bg-slate-900/90 backdrop-blur-md border border-slate-600 hover:bg-blue-600 text-white font-bold text-[11px] px-3 py-2 rounded-lg flex items-center gap-1.5 shadow-2xl transition-all">
                                    ...
                                    {upscalingIndex === null && (
                                        <div className="absolute bottom-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20 pointer-events-none group-hover:pointer-events-auto">
                                            <button onClick={() => handleUpscale('2k')} className="bg-slate-900/90 backdrop-blur-md border border-slate-600 hover:bg-blue-600 text-white font-bold text-[10px] px-3 py-1.5 rounded-md shadow-2xl transition-all">UPSCALE 2K</button>
                                            <button onClick={() => handleUpscale('4k')} className="bg-slate-900/90 backdrop-blur-md border border-slate-600 hover:bg-blue-600 text-white font-bold text-[10px] px-3 py-1.5 rounded-md shadow-2xl transition-all">UPSCALE 4K</button>
                                        </div>
                                    )}
                                </div>
                            ) : (
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            {!isLoading && resultImages.map((image, index) => (
                                <div
                                    key={index}
                                    className={`relative group aspect-square bg-slate-700 rounded-lg overflow-hidden cursor-pointer transition-all duration-200 ${selectedImageIndex === index ? 'ring-2 ring-offset-2 ring-offset-slate-800 ring-blue-500' : 'opacity-70 hover:opacity-100'}`}
                    ...
                    <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 shadow-2xl p-6 rounded-xl">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-lg font-semibold text-slate-300 flex items-center gap-2"><Icon name="clock" className="w-5 h-5 text-blue-400" /> Lịch sử {task.name}</h2>
                            {utilityHistory.length > 0 && (
                                <button onClick={() => { if(confirm("Xóa toàn bộ lịch sử?")) { setUtilityHistory([]); localStorage.removeItem(`utility_history_${task.id}`); }}} className="text-red-400 hover:text-red-500 text-xs font-bold flex items-center gap-1 transition-colors"><Icon name="trash" className="w-4 h-4" /> XÓA TẤT CẢ</button>
                            )}
                        </div>
                        {utilityHistory.length > 0 ? (
                            <div className="space-y-3 overflow-y-auto max-h-72 pr-2 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                                {utilityHistory.map((item) => (
                                    <div key={item.id} onClick={() => {setResultImages(item.images); setSelectedImageIndex(0);}} className="flex items-center gap-4 bg-slate-700/30 p-2.5 rounded-xl hover:bg-slate-700/60 cursor-pointer transition-all border border-transparent hover:border-slate-600/50">
                                        <div className="w-14 h-14 shrink-0 rounded-lg overflow-hidden shadow-lg"><img src={item.images[0]} className="w-full h-full object-cover" /></div>
                                        <div className="flex-grow min-w-0">
                                            <div className="flex justify-between items-center mb-0.5"><p className="font-bold text-xs text-slate-200">{item.images.length} ảnh</p><span className="text-[10px] font-orbitron text-slate-500 bg-black/20 px-1.5 py-0.5 rounded">{item.timestamp}</span></div>
                                            <p className="text-[11px] text-slate-400 truncate leading-relaxed" title={item.prompt}>{item.prompt}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
...
export const UtilitiesTab: React.FC<{ initialTaskId?: string; onEditRequest: (imageUrl: string) => void; isStandalone?: boolean; }> = ({ initialTaskId, onEditRequest, isStandalone }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {UTILITY_TASKS.map(t => (
                <div key={t.id} onClick={() => setSelectedTask(t)} className="bg-slate-800/40 border border-slate-700/50 p-6 rounded-2xl hover:border-blue-500 hover:bg-slate-800/60 cursor-pointer transition-all shadow-xl group border-b-4 border-b-transparent hover:border-b-blue-500 relative overflow-hidden">
                    <div className="absolute -right-4 -top-4 text-blue-500/5 rotate-12 group-hover:scale-125 transition-transform duration-700 pointer-events-none"><Icon name={t.icon} className="w-32 h-32" /></div>
                    <div className="text-blue-500 mb-5 transition-transform group-hover:scale-110 group-hover:-rotate-3 relative z-10"><Icon name={t.icon} className="w-10 h-10" /></div>
                    <h3 className="font-bold text-slate-100 text-lg mb-2 relative z-10">{t.name}</h3>
                    <p className="text-sm text-slate-400 leading-relaxed relative z-10">{t.description}</p>
                    <div className="mt-6 flex items-center text-blue-400 text-[11px] font-bold tracking-widest uppercase opacity-0 group-hover:opacity-100 transition-all relative z-10">SỬ DỤNG NGAY <Icon name="arrow-right" className="w-3 h-3 ml-2" /></div>
                </div>
            ))}
        </div>
    );
