...
import { generateImages, upscaleImage, describeInteriorImage } from './services/geminiService';
...
const ResultDisplay: React.FC<{
  ...
  return (
    <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 shadow-2xl shadow-black/25 p-6 rounded-xl h-full flex flex-col relative z-0">
          ...
          <div className="relative group w-full h-full flex items-center justify-center">
            ...
            {upscalingIndex === null && (
              <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                ...
                <button onClick={() => onEditRequest(selectedImage)} className="bg-slate-800/80 backdrop-blur-sm border border-slate-600 hover:bg-blue-600 text-white font-bold text-xs px-3 py-2 rounded-md transition-colors flex items-center gap-1.5">
                  <Icon name="pencil" className="w-4 h-4" />
                  <span>Sửa</span>
                </button>
                {showChangeAngleButton && (
                  <button onClick={() => onChangeAngle(selectedImageIndex)} className="bg-slate-800/80 backdrop-blur-sm border border-slate-600 hover:bg-blue-600 text-white font-bold text-xs px-3 py-2 rounded-md transition-colors flex items-center gap-1.5">
            ...
            {upscalingIndex === null && (
              <div className="absolute bottom-2 right-2 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                <button onClick={() => onUpscale(selectedImageIndex, '2k')} className="bg-slate-800/80 backdrop-blur-sm border border-slate-600 hover:bg-slate-700 text-white font-bold text-xs px-2 py-1 rounded-md transition-colors">UPSCALE 2K</button>
                <button onClick={() => onUpscale(selectedImageIndex, '4k')} className="bg-slate-800/80 backdrop-blur-sm border border-slate-600 hover:bg-slate-700 text-white font-bold text-xs px-2 py-1 rounded-md transition-colors">UPSCALE 4K</button>
              </div>
            )}
          </div>
        ) : (
...
const UpscaleModal: React.FC<{ imageUrl: string; onClose: () => void; }> = ({ imageUrl, onClose }) => {
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[200] p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl max-w-4xl max-h-[90vh] flex flex-col relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute -top-4 -right-4 bg-blue-600 text-white rounded-full p-2 hover:bg-blue-700 transition-transform duration-200 z-10"><Icon name="x-mark" className="w-6 h-6" /></button>
...
export default function App() {
  ...
  const [interiorAspectRatio, setInteriorAspectRatio] = useState<"original" | "1:1" | "3:4" | "4:3" | "9:16" | "16:9">("original");
  ...
  const [colorFloorplanPrompt, setColorFloorplanPrompt] = useState('Render một phiên bản chất lượng cao của floorplan 2D này with góc nhìn từ trên xuống. Thêm vật liệu thực tế và bóng đổ mềm.');
  const [colorFloorplanAspectRatio, setColorFloorplanAspectRatio] = useState<"original" | "1:1" | "3:4" | "4:3" | "9:16" | "16:9">("original");
  ...
  useEffect(() => {
    ...
    try {
      const eH = localStorage.getItem('exteriorRenderHistory'); if (eH) setExteriorHistory(JSON.parse(eH));
      const iH = localStorage.getItem('interiorRenderHistory'); if (iH) setInteriorHistory(JSON.parse(iH));
      const fH = localStorage.getItem('floorplanHistory'); if (fH) setFloorplanHistory(JSON.parse(fH));
      const cfH = localStorage.getItem('colorFloorplanHistory'); if (cfH) setColorFloorplanHistory(JSON.parse(cfH));
      const edH = localStorage.getItem('editHistory'); if (edH) setEditHistory(JSON.parse(edH));
    } catch (error) { console.error("Failed to load history", error); }
  }, []);

  useEffect(() => safeSaveToLocalStorage('exteriorRenderHistory', exteriorHistory), [exteriorHistory]);
  useEffect(() => safeSaveToLocalStorage('interiorRenderHistory', interiorHistory), [interiorHistory]);
  useEffect(() => safeSaveToLocalStorage('floorplanHistory', floorplanHistory), [floorplanHistory]);
  useEffect(() => safeSaveToLocalStorage('colorFloorplanHistory', colorFloorplanHistory), [colorFloorplanHistory]);
  useEffect(() => safeSaveToLocalStorage('editHistory', editHistory), [editHistory]);
  ...
  const handleGeneration = useCallback(async (prompt: string, renderType: 'exterior' | 'interior' | 'floorplan' | 'color-floorplan') => {
    ...
    try {
      ...
      else if (renderType === 'floorplan') setFloorplanHistory(prev => [newHistoryItem, ...prev]);
      else if (renderType === 'color-floorplan') setColorFloorplanHistory(prev => [newHistoryItem, ...prev]);
    } catch (error) { handleHandleError(error); } finally { setIsLoading(false); }
  }, [sourceImage, referenceImage, exteriorAspectRatio, interiorAspectRatio, floorplanAspectRatio, colorFloorplanAspectRatio, handleHandleError]);
  ...
  const selectCommonStyles = "w-full bg-slate-700 p-3 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none appearance-none";
  ...
  return (
    <div className="min-h-screen p-8 relative">
      <header className="text-center mb-10">
        <div className="inline-block border-2 border-[#1e3a8a] px-6 py-2 shadow-[0_0_15px_rgba(30,58,138,0.5)] bg-[#0d1425]">
          <h1 className="text-3xl md:text-4xl font-bold tracking-wider text-slate-100 uppercase font-orbitron">
            AI RENDER FOR <span className="text-blue-500">ARCHITECTURE - INTERIOR</span>
          </h1>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex border-b border-slate-700 mb-8 overflow-x-auto">
          ...
          <TabButton label="Render Nội Thất" icon="home" isActive={activeTab === 'interior'} onClick={() => setActiveTab('interior')} />
          <TabButton label="Floorplan to 3D" icon="cube" isActive={activeTab === 'floorplan'} onClick={() => setActiveTab('floorplan')} />
          <TabButton label="Render Mặt bằng" icon="brush" isActive={activeTab === 'color-floorplan'} onClick={() => setActiveTab('color-floorplan')} />
          <TabButton label="Chỉnh Sửa Ảnh" icon="pencil" isActive={activeTab === 'edit'} onClick={() => setActiveTab('edit')} />
        ...
        <main>
          ...
          {activeTab === 'interior' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 flex flex-col gap-8">
                <ResultDisplay images={generatedImages} isLoading={isLoading} onUpscale={handleUpscale} upscalingIndex={upscalingIndex} onEditRequest={handleEditRequest} selectedImageIndex={selectedImageIndex} onSelectImageIndex={setSelectedImageIndex} onChangeAngle={handleChangeAngle} onFullscreen={(index) => setImageForFullscreen(generatedImages[index])} showChangeAngleButton={true} />
                <HistoryPanel title="Lịch Sử Render Nội Thất" history={interiorHistory} onClear={() => setInteriorHistory([])} onSelect={(item) => { setGeneratedImages(item.images); setSelectedImageIndex(0); }} />
              </div>
            </div>
          )}
